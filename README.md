...
to quantify what changed between **0.3.4** and **0.7.1**.
...
sequential — and **0.7.1 must go first** (see below).

```bash
# 1. 0.7.1 — also writes fixtures/rca_references.json
#    pyproject: multimodars[meshlab]==0.7.1
...
| File | What it shows |
| --- | --- |
| `per_step.png` | one linear panel per block, 0.3.4 vs 0.7.1 |
...
Each box covers every case that version measured, with its own n printed
underneath — `stitching` and `post_processing` ran on all 14 cases in 0.7.1 but
only on the 3 that survived 0.3.4, and restricting 0.7.1 to the shared 3 would
report its runtime over a case set it never failed on. Only the P values are
restricted to the complete pairs.

Blocks one version crashed on are labelled in red with that version's failure
count, and the totals come in two flavours: end to end, where each version
covers the cases it finished, and the per-case sum over only the blocks no case
failed, so that one is backed by every case in both versions.

## Why 0.7.1 has to run first

`align_combined` needs an (aortic, superior, inferior) reference triplet at the
coronary ostium. 0.7.1 derives it from `discretize_vessel_tree`
(`tree.rca_references[0]`); 0.3.4 has no such function, and the real v0.3.4
workflow hardcoded the three points per case by hand.

So the 0.7.1 run caches its triplets to `fixtures/rca_references.json` and the
...
The 0.3.4 pipeline mirrors the 0.7.1 one block for block. Where 0.3.4 lacks a
capability, the gap is closed in `compat_0_3_4.py` (reimplementing 0.7.1's Rust
semantics in numpy) and marked `# 0.3.4:` at the call site:

| 0.7.1 | 0.3.4 | Stand-in |
| --- | --- | --- |
...
| branch model (`calculate_branches`, `remove_branch_overlap`, `get_branch`) | none — a centerline is one flat polyline | only the main branch survives; 0.7.1's VTP reader sorts branches by descending arc length, so "main" is the *longest* polyline, not the first in the file |
...
| `align_combined(..., align_wall_anomalous=True) -> (geom, spacing_mm, rotation)` | no such flag; returns `(geom, resampled_cl)` | `spacing_mm` recomputed from the mean distance between consecutive lumen centroids, which is how 0.7.1 defines it |
...
`clamp_overshoot` arguments added later are what give 0.7.1 control over it.
